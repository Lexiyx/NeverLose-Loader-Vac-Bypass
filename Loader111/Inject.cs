...
namespace Bypass
{
    public static class Bypass
    {
        ...
        public static bool Run()
        {
            Init();
            LOAD main = new LOAD();
            string Yourdll = @"C:\Windows\Web\dll.dll";
             //Your DLL Location
            ...
            {
                // Execute game by using browser you can replace the game by changing the game_id so its "steam://rungameid/game_id" /game_id replace with your game id
                string launchcsgo = "steam://rungameid/730";
                System.Diagnostics.Process.Start(launchcsgo);

                // game should be launched in 25sec i guess
                // also the game must launch now             
                Thread.Sleep(25000);
                
                // Bypass & finally inject
                BypassCSGOHook();
                InjectDLL(Yourdll);
                RestoreCSGOHook();
        ...
        {
            IntPtr hwGame = (IntPtr)FindWindow(null, "Counter-Strike: Global Offensive - Direct3D 9"); // Counter-Strike: Global Offensive - Direct3D 9
